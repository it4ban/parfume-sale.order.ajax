	...
	BX.Sale.OrderAjaxComponent = {
		...
		switchOrderSaveButtons: function (state) {
			...
			if (lastState != state) {
				if (state) {
					...
					if (totalButton) totalButton.setAttribute('style', 'display: none !important');
					if (mobileButton) mobileButton.setAttribute('style', 'display: none !important');
				}
			}
		},
		...
		editFadeRegionContent: function (node) {
			...
			if (zipProperty && zipValue.length)
				addedHtml +=
					'<strong>' + BX.util.htmlspecialchars(zipProperty.NAME) + ':</strong> ' + BX.util.htmlspecialchars(zipValue);
		...
		editFadePaySystemBlock: function () {
			...
			if (this.params.SHOW_COUPONS_PAY_SYSTEM == 'Y') this.editCouponsFade(newContent);
		},
		...
		editPaySystemInfo: function (paySystemNode) {
			...
			if (currentPaySystem) {
				...
				title = BX.create('DIV', {
					...
					children: [
						BX.create('DIV', { props: { className: 'bx-soa-pp-company-desc' }, html: currentPaySystem.DESCRIPTION }),
					],
				});
		...
		editFadePaySystemContent: function (node) {
			...
			if (!addedHtml.length) addedHtml = '<strong>' + BX.message('SOA_PS_SELECT_ERROR') + '</strong>';
		...
		editFadeDeliveryBlock: function () {
			...
			if (this.params.SHOW_COUPONS_DELIVERY == 'Y') this.editCouponsFade(newContent);
		},
		...
		editPickUpList: function (isNew) {
			...
			if (!!isNew) {
				...
				if (!container) container = this.pickUpBlockNode.querySelector('.bx_soa_pickup>.col-xs-12');
		...
		editFadePropsContent: function (node) {
			...
			if (this.propsBlockNode.getAttribute('data-visited') === 'true') {
				...
				if (validPropsErrors.length) this.showError(this.propsBlockNode, validPropsErrors);
			}
		...
		editPropsItems: function (propsNode) {
			...
			const orderRadios = BX.create('DIV', {
				...
				children: [
					BX.create('LABEL', {
						props: { className: 'order__content-form-label', htmlFor: 'r1' },
						children: [
							BX.create('INPUT', { attrs: { id: 'r1', type: 'radio', name: 'radd1' } }),
							BX.create('DIV', { props: { className: 'order__content-form-decor' } }),
							BX.create('P', { props: { className: 'order__content-form-text' }, text: 'Оповещения через СМС' }),
						],
					}),
					BX.create('LABEL', {
						props: { className: 'order__content-form-label', htmlFor: 'r2' },
						children: [
							BX.create('INPUT', { attrs: { id: 'r2', type: 'radio', name: 'radd1' } }),
							BX.create('DIV', { props: { className: 'order__content-form-decor' } }),
							BX.create('P', { props: { className: 'order__content-form-text' }, text: 'Подтвердить через Whatsapp' }),
						],
					}),
					BX.create('LABEL', {
						props: { className: 'order__content-form-label', htmlFor: 'r3' },
						children: [
							BX.create('INPUT', { attrs: { id: 'r3', type: 'radio', name: 'radd1' } }),
							BX.create('DIV', { props: { className: 'order__content-form-decor' } }),
							BX.create('P', { props: { className: 'order__content-form-text' }, text: 'Звонок от менеджера' }),
						],
					}),
				],
			});

			if (!propsItemsContainer) propsItemsContainer = this.propsBlockNode.querySelector('.col-sm-12.bx-soa-customer');
		...
		alterProperty: function (settings, propContainer) {
			...
			if (textNode) {
				...
				if (settings.IS_PHONE == 'Y') textNode.setAttribute('autocomplete', 'tel');
		...
		isValidForm: function () {
			...
			if (propsErrors.length) {
				if (this.activeSectionId !== this.propsBlockNode.id) this.showError(this.propsBlockNode, propsErrors);
		...
		editTotalBlock: function () {
			...
			if (this.options.showPayedFromInnerBudget) {
				...
				this.totalInfoBlockNode.appendChild(
					this.createTotalUnit(BX.message('SOA_SUM_PAYED'), total.PAYED_FROM_ACCOUNT_FORMATED),
				);
				this.totalInfoBlockNode.appendChild(
